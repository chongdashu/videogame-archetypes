...
raw <- read.arff(file='../arff/dota2_heroes.arff') 

# Separate out names.
names <- raw$Hero
primaryAttr <- raw[, c('Primary-Attr')]

attackType <- raw[, c('Attack-range')]
attackType[raw$'Attack-range' == 128] = 'Melee'
attackType[raw$'Attack-range' != 128] = 'Ranged'
attackType = as.factor(attackType)

# Construct reduced data, without names.
data <- raw[, -which(names(raw) %in% c("Hero","Primary-Attr"))]
...
ternaryplot(coef(model, 'alphas'), col = 2*as.numeric(attackType), id=names, dimnames=c("1","2", "3"), cex=0.8, dimnames_position = c('corner'), labels = c('inside'), main="Archetypal Heroes  in Dota 2")
